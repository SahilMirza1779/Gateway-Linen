...
/*
|--------------------------------------------------------------------------
| FETCH ONLY SUB-CATEGORIES (CHILD CATEGORIES ONLY)
|--------------------------------------------------------------------------
*/
...
$categorySql = "
    ...
    LEFT JOIN dbo.Categories parent ON child.ParentCategoryId = parent.CategoryId
    WHERE child.ParentCategoryId IS NOT NULL AND child.ParentCategoryId > 0
...
.add-form-input, .add-form-select { height: 42px; padding: 0 13px; }
.add-form-textarea { min-height: 90px; padding: 10px 13px; resize: vertical; line-height: 1.5; }
...
.tax-input-wrap .add-form-input {
    flex: 1;
}

.tax-input-wrap .add-form-input.is-locked {
    background: rgba(13, 22, 32, 0.7);
    color: var(--amber);
    font-weight: 700;
            ...
            <div class="add-category-header">
                <div>
                    ...
                    <p class="add-category-subtitle">
                        Create a product with pricing, tax (GST 5% & PST 7% default), multiple images, specifications, and SEO settings.
                    </p>
                </div>
                    ...
                    <div class="add-category-grid">
                        ...
                        <div class="add-form-group">
                            <label for="productCategory" class="add-form-label">
                                Sub-Category <span class="add-form-required">*</span>
                            </label>
                                ...
                                <?php if (!empty($categories)): ?>
                                    <?php foreach ($categories as $cat): ?>
                                        <option value="<?= (int)$cat["CategoryId"] ?>" <?= (string)$categoryId === (string)$cat["CategoryId"] ? "selected" : "" ?>>
                                            [<?= e($cat["ParentName"] ?? 'Category') ?>] › <?= e($cat["ChildName"]) ?>
                                        </option>
                                    <?php endforeach; ?>
                                <?php else: ?>
                                    <option value="" disabled>No Sub-Categories available. Please add one first.</option>
                                <?php endif; ?>
                            </select>
                            <div class="help-text" style="font-size:10px; color:var(--text-mute); margin-top:5px;">
                                Only child sub-categories are selectable.
                            </div>
                        </div>
                        ...
                        <div class="add-form-group">
                            ...
                            <input type="number" step="0.01" min="0" id="basePrice" name="base_price" class="add-form-input" value="<?= e($basePrice) ?>" placeholder="0.00" required>
                        </div>

                        <!-- GST PERCENTAGE (DEFAULT 5% LOCKED WITH EDIT BUTTON) -->
                        <div class="add-form-group">
                            ...
                            <div class="tax-input-wrap">
                                <input 
                                    ...
                                    id="gstPercentage" 
                                    name="gst_percentage" 
                                    class="add-form-input is-locked" 
                                    value="<?= e($gstPercentage) ?>" 
                                    readonly
                                >
                                <button type="button" class="btn-tax-toggle" id="btnToggleGst" title="Edit GST">
                                    <span>✎</span> <span>Edit</span>
                                </button>
                            </div>
                        </div>

                        <!-- PST PERCENTAGE (DEFAULT 7% LOCKED WITH EDIT BUTTON) -->
                        <div class="add-form-group">
                            ...
                            <div class="tax-input-wrap">
                                <input 
                                    ...
                                    readonly
                                >
                                <button type="button" class="btn-tax-toggle" id="btnTogglePst" title="Edit PST">
                                    <span>✎</span> <span>Edit</span>
                                </button>
                            </div>
                        </div>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Tax Lock/Unlock Toggle Functionality
    function setupTaxToggle(inputId, btnId) {
        ...
        btn.addEventListener("click", function(e) {
            ...
            if (input.hasAttribute("readonly")) {
                ...
                btn.innerHTML = "<span>🔒</span> <span>Lock</span>";
            } else {
    ...
    setupTaxToggle("gstPercentage", "btnToggleGst");
    setupTaxToggle("pstPercentage", "btnTogglePst");
            ...
            if ($newProductId) {
                ...
                foreach ($uploadedFiles as $index => $uFile) {
                    if (move_uploaded_file($uFile["tmp"], $uFile["dest"])) {
                        ...
                        $isMain = ($index === 0) ? 1 : 0; // Pehli image main banegi
                        $displayOrder = $index + 1;
                        $altText = $name;
